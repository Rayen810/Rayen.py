...
def generate_passwords(fullname: str) -> List[str]:
    ...
    # النمط الثاني: first_name (الحرف الأول كبير) + أرقام
...
class InstagramLogin:
    ...
    def login(self, username: str, password: str, user_agent: str, mid: str, csrf_token: str) -> Dict[str, str]:
        ...
        try:
            ...
            response_text = response.text[:2800]  # Limit response text to avoid memory issues
    ...
    def process_accounts(self, accounts: List[Tuple[str, List[str]]], max_workers: int = 5) -> None:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                with self.lock:
                    self.loop_count += 1
                    KenXinDev(f'[bold bright_black]   ──> [bold white]𝙲𝚛111𝚊𝚌𝚔 \033[1;36m {self.loop_count}[bold white]/\033[1;36m{len(accounts)} [bold white]| [bold white]𝙾𝚔 \033[1;30m: \033[1;32m{len(self.success)} [bold white]|  [bold white] 𝙲𝚙\033[1;30m :\033[1;33m \033[1;33m{len(self.checkpoint)}[bold white]  [/]', end='\r')  
...
def main():
    ...
    print(f"{Fore.CYAN}[*] Lo++7aded {len(accounts)} accounts{Style.RESET_ALL}")
